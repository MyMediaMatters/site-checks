...
/**
 * @param {{root?: string, exclude?: string[], include?: string[], extraRoutes?: string[]}} [options]
...
export function discoverRoutes(options = {}) {
  ...
  // `include` replaces discovery outright; `extraRoutes` adds to it. Dynamic
  // routes ([slug]) cannot be enumerated from disk, so a representative
  // instance is added via extraRoutes rather than by abandoning discovery.
  if (options.include?.length) return [...new Set([...options.include, ...extra])].sort();
  ...
  for (const r of extra) routes.add(r);
  routes.add("/");
  return [...routes].sort();
}
